...
import random4_map
from Tkinter import Tk, Button, Label, Entry, StringVar, IntVar, Listbox, END, ANCHOR, E, W
...
def main(argv):
    if len (argv) == 1:
        ...
        Label(root, text="Output map:").grid(row=5, sticky=E)
        Entry(root, width=50, textvariable=outfilevar).grid(row=5, column=1)
        ...
        indylist = Listbox(root, width=50, height=5)
        indylist.grid(row=1, column=1, rowspan=4)
        ...
        Button(root, width=10, text="Generate!", \
            command=lambda : runGenerator(mapfilevar.get(), indylist.get(0, END), outfilevar.get(), landstartvar.get(), waterstartvar.get(), \
                ...
                lvl2thronevar.get(), lvl3thronevar.get(), rarechancevar.get(), indystrengthvar.get())).grid(row=10, column=3)
        ...
        runGenerator(mapfile, [indyfile], targetfile, landcount, watercount, coaststarts, foreststarts, swampstarts, cavestarts, lvl1thrones, lvl2thrones, lvl3thrones, rarechance, strength)


def runGenerator(mapfile, indyfiles, targetfile, landcount, watercount, coaststarts, foreststarts, swampstarts, cavestarts, lvl1thrones, lvl2thrones, lvl3thrones, rarechance, strength):
